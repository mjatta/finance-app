...
export default function IncomeStatement() {
  ...
  const convertToCSV = (rows) => {
    const headers = ['Category', 'Subgroup', 'Account Number', 'Account Name', 'Debit', 'Credit'];
  ...
  const handleExportCSV = (data) => {
    ...
    const filename = `income-statement-${date}.csv`;
    downloadFile(csvContent, filename, 'text/csv');
  };

  const handleExportExcel = (data) => {
    const csvContent = convertToCSV(data);
    const filename = `income-statement-${date}.xlsx`;
  ...
  const handleFetchAndExport = async (exportType) => {
    ...
    const data = await fetchIncomeStatement(branchId === 'ALL' ? '' : branchId, date);
        ...
        <CardContent sx={{ p: 3 }}>
          ...
          <Box sx={{ display: 'grid', gap: 2, gridTemplateColumns: { xs: '1fr', md: 'repeat(2, minmax(0, 1fr))' } }}>
            <TextField
              ...
              SelectProps={{ displayEmpty: true, renderValue: (selected) => (selected === 'ALL' || !selected) ? 'All Branches' : (branchOptions.find((o) => o.id === selected)?.name || 'All Branches') }}
            >
              <MenuItem value="ALL">
                All Branches
              </MenuItem>
