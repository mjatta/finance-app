...
import { useGetLoginAttempts } from './hooks/useGetLoginAttempts';
...
export default function LoginAttempts() {
  const { attempts, loading, error, refetch } = useGetLoginAttempts();
    ...
    { field: 'reason', headerName: 'Reason', flex: 1, minWidth: 150, valueFormatter: (value) => value || '-' },
    {
      ...
      renderCell: (params) => (
        <Button
          ...
          onClick={() => handleDownloadLog(attempts.find((l) => l.id === params.row.id))}
        >
          JSON
        </Button>
      ),
  ...
  return (
    <Box p={3}>
      ...
      <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
        {loading ? 'Loading login attempts from server...' : 'Displaying login attempts from the system'}
      </Typography>
      ...
      <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1.5} sx={{ mb: 2 }}>
        <Card sx={{ minWidth: 180, border: '1px solid', borderColor: 'divider' }}>
          <CardContent sx={{ py: 1.5, '&:last-child': { pb: 1.5 } }}>
            ...
            <Typography variant="h6" sx={{ fontWeight: 700 }}>{attempts.length}</Typography>
          </CardContent>
        </Card>
        <Card sx={{ minWidth: 180, border: '1px solid', borderColor: 'divider' }}>
          <CardContent sx={{ py: 1.5, '&:last-child': { pb: 1.5 } }}>
      ...
      <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1.5} sx={{ mb: 2 }}>
        ...
        <Button variant="outlined" startIcon={<DownloadIcon />} onClick={handleExport} disabled={attempts.length === 0}>
          Export JSON
        </Button>
      </Stack>
      ...
      {error && (
        <Card sx={{ mb: 2, p: 2, backgroundColor: '#fee', borderLeft: '4px solid #ef4444' }}>
          <Typography color="error">
            Error loading login attempts: {error}
          </Typography>
        </Card>
      )}
