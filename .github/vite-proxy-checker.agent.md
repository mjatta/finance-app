...
## Purpose
This agent ensures that any new backend API endpoints used in the codebase are properly configured in the Vite proxy section of vite.config.js. This prevents CORS and 404 issues during local development.
...
5. **Optionally**: Offer to auto-insert the missing proxy entry if user approves.

## Usage
- Run this agent after adding new API endpoints or when encountering CORS/404 issues in development.
- The agent will output a list of missing proxy entries and the exact code to add to vite.config.js.
